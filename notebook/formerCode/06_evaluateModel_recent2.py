...
class Decoder(nn.Module):
    ...
    # ★ 05 に合わせて (dec_in, h0, c0) を受け取り、h0,c0 を LSTM1 に渡す
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src: torch.Tensor, tgt_len: int) -> torch.Tensor:
        ...
        # ★ 出力は常に output_dim。time_context を足した入力次元は使わない。
        y_prev = torch.zeros(B, 1, self.output_dim, device=src.device, dtype=src.dtype)
        outs = []
        for _ in range(tgt_len):
            y_hat = self.decoder(y_prev, h, c)  # ★ 05 と同じ呼び方
            outs.append(y_hat)
            y_prev = y_hat.detach()
...
def load_data():
    ...
    raw_measured = np.load('measured_training_data_sameRowColSeq31.npy')
    input_data = safe_inverse_k(raw_measured)

    united = np.load('united_triangular_matrices.npy')
    initial_data = safe_inverse_k(united[:, 0, :, :])        # ★ ← ここも safe に
    ...
    output_data = safe_inverse_k(united)                     # ★ ← ここも safe に
