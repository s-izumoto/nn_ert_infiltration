...
本リポジトリでは、OpenFOAMにより土壌表層への水の浸透過程を高時間分解能で数値的に再現し、得られた結果をもとにERTの時間的応答を解析することで、**短時間で変化する表層浸潤を可視化・補間するための物理的基盤**を提供します。
...
本リポジトリでは、OpenFOAM を用いて**土壌中への水の浸潤過程を時間発展的にシミュレーション**し、その結果として得られる**各セルの水分量・塩分濃度・飽和度分布**をもとに、**電気伝導率（σ）マップ**を生成します。  
