# ERT — 土壌への水（溶質）浸透 + ニューラル補完

OpenFOAM による物理ベースのシミュレーションと LSTM/Seq2Seq モデルを組み合わせ、電気探査（ERT: Electrical resistivity tomography）測定で得られた**土壌への水（溶質）浸透中の見かけ比抵抗場**を高解像度・時系列的に再構成します。元の ERT 測定データを補間することで**時間／空間分解能が約15倍向上**することを期待できます。実際の結果例は、`movie/` フォルダー内の動画で確認できます。
> ℹ️ 用語の背景（ERT、浸透、逆解析、電極配列など）の概要説明は、本 README の最後にある **「📘 背景説明（Background）」セクション** を参照してください。

---

## 要約 (TL;DR)
- **目的:** 合成データで学習したニューラルネットワークを用いて、**土壌への水（溶質）浸透**過程の ERT 測定における空間的・時間的ギャップを補完すること。より具体的には、測定範囲のみ更新された測定マップから全領域の真値マップを推定する。
...
- **浸潤条件:** 土壌表面中央、直径 30 cm の円内部を **0.0885 mol/L NaCl** 溶液で常時飽和状態に維持  

---

## **OpenFOAM 出力 → NumPy 入力（`.npy`）仕様**

本ワークフローでは、OpenFOAM などで事前にシミュレーションされた浸潤過程の結果を、NumPy 配列ファイル（`.npy`）として入力します。`01_generateAppRes.py` はこのファイルを読み込み、pyGIMLi による Wenner–alpha 配列の順解析を行います。

---

### **ドメイン（物理サイズ）**
- OpenFOAMのシミュレーション領域は **幅 3.0 m × 高さ 1.0 m × 奥行き 1.0 m**（**y ∈ [0, 3.0] m**, **x ∈ [0, 1.0] m**, **z ∈ [0, 1.0] m**）です。
  z = 0.5 における断面をpyGIMLiのシミュレーション領域として使います。
- pyGIMLiのシミュレーション領域は **幅 3.0 m × 高さ 1.0 m**（**y ∈ [0, 3.0] m**, **x ∈ [0, 1.0] m**）です。  
...
**目的:**  
`01_generateAppRes.py`で生成された複数の**三角行列見かけ比抵抗ファイル**（例：`triangular_matrix_seq_000.npy`）を読み込み、形状を確認したうえで**学習用／テスト用データセット**に分割・統合します。  
...
**目的:**  
`02_uniteTriangular.py`で統合した**真値（基準）比抵抗マップ**をもとに、時系列ごとに「真値と測定値の差が大きい位置」を選び出し、その場所を新たに測定する過程をシミュレーションして **測定値データ（measured）** と **測定位置データ（indices）** を生成します。  
...
**目的:**  
`03_generateMeasDesign.py` で得られた**測定設計（位置情報）**と、`02_uniteTriangular.py` で統合された**真値比抵抗マップ**を用いて、時系列ごとに「同じ測定パターンを適用した測定値マップ（measured）」を生成します。これにより、すべての時系列データで一貫した測定プロセスを再現し、**学習用データ**を作成します。  
...
**目的:**  
`04_generateTrainingData.py` で作成した **測定マップ（measured）** と、`02_uniteTriangular.py` の **真値マップ（united）** を用いて、**短い履歴ウィンドウから将来の真値（三角行列マップ）を予測**する 2 層 LSTM の Encoder–Decoder（Seq2Seq）モデルを学習します。  
...
3. **エンコーダ用の履歴長 `time_steps` とデコーダ出力長 `output_seq_length`** に基づき、各時系列データから **30 ステップ分の過去データを入力として切り出し、直後の 29 ステップを予測する学習サンプルを作成**。デフォルト設定（`trainingSequence.yml`）では、この 1 セットのみを使用して **「30 ステップの履歴から、次の 29 ステップの真値の変化（Δ）を予測する」** 学習を行います。  
...
4. **学習と評価**  
   - 学習率やバッチサイズの組み合わせを変えてモデルを複数回学習し、**検証データに対する誤差の平均値が最も小さい設定**を自動的に選択  
...
**目的:**  
`05_trainingSequence.py` で学習した **LSTM Encoder–Decoder（Seq2Seq）モデル** を用いて、**30 ステップの履歴**から **次の 29 ステップの真値マップの変化（Δ）** を**自己回帰的（autoreg）** に連続推論します。入力は `04_generateTrainingData.py` で生成した **測定マップ（measured）** と、初期化および正規化参照のための **真値マップ（united）** を使用します。
...
**目的:**  
`04_generateTrainingData.py` で生成した **測定マップ（measured）** と、`02_uniteTriangular.py` の **真値マップ（united）** を用いて、**最初の時間差分 Δ(t=0→1)** を予測する **単一出力 LSTM 回帰モデル** を学習します。全時系列を扱う `05_trainingSequence.py` の短期版にあたり、最初の変化のみを対象として学習を行います。  
...
**目的:**  
`08_inferFirst.py`（t=0→1 の Δ）および `06_inferSequence.py`（以降の Δ 連続予測）の結果を用いて、**初期フレーム t=0** から **Δ（差分）を時間方向に累積**し、**全時系列の伝導率／抵抗率マップ**を再構成します。再構成結果を**真値データ**と整合させて評価（MAPE）し、必要に応じて比較画像を出力します。
...
**目的:**  
全系列・全時刻に対して、**予測マップ（Predicted）** と **測定マップ（Measured）** を**同一カラースケール**で並置し、時間対応をとった PNG 画像として一括出力します（単位は図中ラベルどおり mS·m⁻¹）。
...
この**電極組み合わせの体系**を「電極配列」と呼び、代表的な配列には以下のようなものがあります：

| 配列名 | 特徴 | 主な用途 |
|:--|:--|:--|
| **Wenner 配列** | 4電極（A–M–N–B）が等間隔で配置される。組み合わせが単純。 | ノイズに強く浅部構造に適する |
| **Schlumberger 配列** | 電流電極（A, B）を広く、電位電極（M, N）を近く配置 | 深部構造に感度が高い |
| **Dipole–Dipole 配列** | 電流ペアと電位ペアを離して配置 | 空間分解能が高いがノイズに弱い |
| **Gradient 配列** | 電流電極を固定し、多数の電位電極で同時計測 | 測定効率が高い |
...
### 見かけ比抵抗（Apparent Resistivity）
ERTの測定で直接得られるのは、電極間の電位差や電流値といった**観測データ**です。  
そこから計算される「**見かけ比抵抗（apparent resistivity）**」は、実際の地下構造を平均化した“見かけ上の”比抵抗です。  
これは地下が均質だと仮定した場合に同じ電流・電位差が得られる理論的な値であり、**逆解析を行う前の一次的な指標**として使われます。  
見かけ比抵抗の空間分布は、実際の比抵抗構造の傾向を反映しており、本リポジトリではこの分布を**pyGIMLiによる順解析**で計算します。

---

### 順解析（Forward Modelling）
**順解析（forward modelling）**とは、既知の物性分布（たとえば導電率マップ）を入力として、  
そこから得られる**観測量（電位差や見かけ比抵抗）**を数値的に計算する手法です。  
逆解析（inversion）の逆のプロセスにあたります。  
本リポジトリでは、OpenFOAMで得られた導電率分布を入力として、pyGIMLiにより**Wenner–alpha 配列での見かけ比抵抗マップ**を計算しています。

---

### 逆解析（Inversion）
ERT測定で得られるのは、各電極組み合わせに対応する**地表での電位差**です。  
しかし、私たちが求めたいのは地下の**比抵抗分布**です。  
観測データを最もよく説明する地下モデルを求める数値計算過程が**逆解析（inversion）**です。  
本リポジトリでは、pyGIMLi を用いて逆解析を行い、**逐次的に更新された測定データ**から**時間的に変化する地下比抵抗分布**を再構成します。

---

### pyGIMLi（Python Geophysical Inversion and Modelling Library）
**pyGIMLi** は、地球物理探査の数値解析と逆解析を行うためのオープンソースライブラリです。  
ERT、誘導分極（IP）、周波数領域分極（SIP）などの手法をサポートし、有限要素法（FEM）に基づいて**電場分布・感度分布・逆解析**を実行します。  
本リポジトリでは pyGIMLi を用いて、**Wenner–alpha 配列によるERTの順解析と逆解析**を数値的に再現し、**OpenFOAMによる浸潤シミュレーション結果を入力**として**見かけ比抵抗マップを生成**します。

---


### 浸潤（Infiltration）
**浸潤**とは、降雨や散水などによって水が**土壌表面から地中へしみ込む過程**を指します。  
特に表層の土壌では水の浸透が非常に速く進むため、通常のERT測定の時間分解能では**浸潤前線の動きをリアルタイムに追跡することが難しい**という課題があります。  
しかし、表層での浸潤特性を捉えることは、**土壌の透水性や浸透能（infiltration capacity）の評価**において極めて重要です。  
本リポジトリでは、OpenFOAMにより土壌表層への水（溶質）の浸透過程を高時間分解能で数値的に再現し、得られた結果をもとにERTの時間的応答を解析することで、**短時間で変化する表層浸潤を可視化・補間するための物理的基盤**を提供します。


---

### OpenFOAM — 浸潤過程の数値シミュレーション  
**OpenFOAM** は、流体力学・多相流・熱・化学輸送などを解析できる**オープンソースの数値流体シミュレーションソフトウェア**です。  
本リポジトリでは、OpenFOAM を用いて**土壌中への水（溶質）の浸潤過程を時間発展的にシミュレーション**し、その結果として得られる**各セルの水分量・塩分濃度・飽和度分布**をもとに、**電気伝導率（σ）マップ**を生成します。  

得られた導電率マップは次のステップで pyGIMLi に渡され、**電気比抵抗（ρ = 1/σ）**に変換されたうえで、**ERT順解析**によって**見かけ比抵抗マップ**を計算します。  
つまり、OpenFOAMは本ワークフローにおける**物理ベースの入力データ生成フェーズ**を担い、地中の「真の状態」を仮想的に再現する役割を果たします。  

---

### 🧩 まとめ
- **ERT（電気比抵抗トモグラフィ）** は、複数の電極を用いて地表から電流を注入し、電位差を測定することで地下の**比抵抗分布**を推定する地球物理学的手法である。  
- **電極配列（Electrode Array）** は、電流電極（A–B）と電位電極（M–N）の組み合わせ方を定める測定設計であり、感度分布や探査深度を決定する重要な要素である。本リポジトリでは**Wenner–alpha 配列**を基準配列として使用している。  
- **OpenFOAM** は、土壌中の水や塩水の**浸潤過程を数値的に再現**し、その結果から**導電率分布（σ）**を生成する流体シミュレーションツールである。  
- **pyGIMLi** は、その導電率分布をもとに**順解析**を行い、各電極配列での**見かけ比抵抗マップ**を計算し、さらに**逆解析**によって地下の比抵抗構造を再構成する。  
- **見かけ比抵抗**は、観測データから直接求められる“見かけ上の”抵抗値であり、逆解析によって真の比抵抗構造を推定するための基礎データである。  
- **浸潤**の際に土壌表層で生じる急速な導電率・比抵抗変化を深層学習で補間・再構成することで、**土壌表面付近の浸透能を定量的に評価**することができる。   
